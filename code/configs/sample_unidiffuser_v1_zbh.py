...
def get_config():
    ...
    config.data_type = 1
    config.nnet_path = '/data/zbh/ckpt/nips2023/models/uvit_v1.pth'

    config.autoencoder = d(
        pretrained_path='/data/zbh/ckpt/nips2023/models/autoencoder_kl.pth',
    )

    config.caption_decoder = d(
        pretrained_path="/data/zbh/ckpt/nips2023/models/caption_decoder.pth",
        hidden_dim=config.get_ref('text_dim')
    )

    config.nnet = d(
        name='uvit_fmri', # uvit_multi_post_ln_v1,
        img_size=64,
        in_chans=4,
        patch_size=2,
        embed_dim=1536,
