...
class ZeroResblock(nn.Module):
    def __init__(self, embed_dim, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True):
        super(ZeroResblock, self).__init__()
        self.zero_conv1d_1 = nn.Conv1d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=bias)
        self.zero_conv1d_2 = nn.Conv1d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=bias)
        self.activate = nn.SiLU()
        self.norm = nn.LayerNorm(embed_dim)

    def forward(self, out_feat, in_feat):
        out_feat = self.zero_conv1d_2(self.zero_conv1d_1(in_feat) + out_feat)
        out_feat = self.activate(out_feat)
...
class ControlNetFMRI(nn.Module):
    def __init__(self, in_dim, embed_dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, norm_layer, use_checkpoint, depth):
        super().__init__()
        # self.zero_conv_fmri = ZeroConv2d(in_channels=1, out_channels=1, kernel_size=(1, 1), stride=1, padding=0, bias=True)
        # self.zero_conv_x = ZeroConv2d(in_channel=)
        ...
        self.out_blocks = nn.ModuleList([
            ZeroResblock(embed_dim, in_channels=1106, out_channels=1106,
            kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True) 
            for _ in range(depth)])
        self.add_norm = nn.ModuleList([norm_layer(embed_dim) for _ in range(depth)])
        self.norm = norm_layer(embed_dim)
    
    def forward(self, x, fmri_c):
        c = self.proj(fmri_c) # b*1*1536
        # c = self.zero_conv_fmri(c)
        x = torch.cat([x, c], dim=1) # b*1106*1536
        skips = []
        ...
        x = self.norm(x)
        return x, mid_feature, out_features
...
class UViT(nn.Module):
    def __init__(self, img_size, in_chans, patch_size, embed_dim=768, depth=12,
        ...
        self.controlnet = ControlNetFMRI(1280, embed_dim, num_heads, mlp_ratio, qkv_bias, 
                                        qk_scale, drop_rate, attn_drop_rate, norm_layer, use_checkpoint, depth//2)
    ...
    def forward(self, img, clip_img, text, fmri, t_img, t_text, data_type):
        ...
        x = x + pos_embed
        x = self.pos_drop(x)
        c_x, c_mid, c_outs = self.controlnet(x, fmri)
        
        # c_x_out, fmri_out = c_x.split((4+num_text_tokens+num_img_tokens, 1), dim=1)
        c_mid_out, _ = c_mid.split((4+num_text_tokens+num_img_tokens, 1), dim=1)
        c_out_f = [f.split((4+num_text_tokens+num_img_tokens, 1), dim=1)[0] for f in c_outs]
        ...
        x = self.mid_block(x) + c_mid_out

        for i, blk in enumerate(self.out_blocks):
            x = blk(x, self.controlnet.add_norm[i](skips.pop()+c_out_f[i]))
            # if i%2==0 and i != 0:
            # x = x + c_out_f[i]
