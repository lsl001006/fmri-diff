...
class AlignConvNet(nn.Module):
    def __init__(self, input_dim, output_dim, in_channel=1, out_channel=1024):
        ...
        self.conv = nn.Sequential(nn.Conv1d(in_channel, self.mid_channel, kernel_size=3, padding=1),
                                  ...
                                  nn.SiLU())
        self.merge_conv = nn.Conv1d(out_channel, out_channel, kernel_size=3, padding=1)
    
    def forward(self, image, fmri_clip, contri_w = 0.9):
        fmri_deal = self.conv(self.mlp(fmri_clip))
        return self.merge_conv((1 - contri_w) * image + contri_w * fmri_deal)
...
class UViT(nn.Module):
    def __init__(self, img_size, in_chans, patch_size, embed_dim=768, depth=12,
        ...
        self.align_conv_model = AlignConvNet(input_dim=512, output_dim=1536, in_channel=1, out_channel=1024) # addition
    ...
    def forward(self, img, clip_img, text, t_img, t_text, data_type):
        ...
        ##### add
        fmri_cond = self.align_conv_model(img_out, ori_clip_img)
        img_out = 0.8*img_out + 4*fmri_cond
