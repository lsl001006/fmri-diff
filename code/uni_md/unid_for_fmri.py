...
def Uni_LSimple(img_0, fmri_clip, text_noise, nnet, schedule, config, device, **kwargs):
    N = schedule.N
    n, eps, xn = schedule.sample(img_0)
    z_out, clip_img_out, text_out = nnet(xn, fmri_clip, text=text_noise, t_img=n, t_text=torch.ones_like(n) * N,
                                         data_type=torch.zeros_like(n, device=device, dtype=torch.int) + config.data_type)
    return mos(eps - z_out)
...
class uni_model(nn.Module):
    ...
    def forward(self, z_img, fmri, text_noise, device):
        fmri = self.cond_stage_model(fmri) # b*1*1280
        # import pdb; pdb.set_trace()
        fmri_clip = self.cond_deal(fmri)
        loss = Uni_LSimple(z_img, fmri_clip, text_noise, self.nnet, self.schedule, self.config, device)
        return loss
...
class uniDM:
    def __init__(self, config, pretrain_mbm_metafile, num_voxels, device, epoch=100):
        ...
        self.optimizer = torch.optim.Adam(itertools.chain(self.model.nnet.multi_layer_model.parameters(), self.model.cond_deal.parameters()), 
                                            lr=2e-5, betas=(0.5, 0.999), weight_decay=0.0001)
    ...
    def model_fmri_fn(self, x, timesteps, fmri_clip):
        z, text = self.split_joint(x)
        self.model.nnet.eval()
        z_out, clip_img_out, text_out = self.model.nnet(z, fmri_clip, text=text, t_img=timesteps, t_text=timesteps,
                                             data_type=torch.zeros_like(timesteps, device=self.device, dtype=torch.int) + self.config.data_type)
        return self.combine_joint(z_out, text_out)

    def generated_image(self, img_encoder, caption_decoder, fmri):
        ...
        def model_fn(x, t_continuous):
            ...
            return self.model_fmri_fn(x, t, fmri_clip)
    ...
    def finetune(self, img_encoder, caption_decoder, dataset, test_dataset, bs1):
        ...
        for e in range(self.epoch):
            ...
            for _dict in dataloader:
                ...
                loss = self.model(z_img, fmri, text_noise, self.device)
                loss = loss.sum()
                ...
                if step % 100 == 0:
                    ...
                    image, prompts = self.generated_image(img_encoder, caption_decoder, test_fmri[0:1, ...])
