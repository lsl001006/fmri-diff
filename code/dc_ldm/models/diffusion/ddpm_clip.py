...
class DDPM(pl.LightningModule):
    ...
    def full_validation(self, batch, state=None):
        ...
        grid, all_samples, state = self.generate(batch, ddim_steps=self.ddim_steps, num_samples=5, limit=10, state=state)
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.clip_img_model, _ = clip.load("/data/zbh/ckpt/clip/ViT-B-32.pt", device='cuda', jit=False)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        image_start = self.decode_first_stage(x_start)
        LDM_image_224 = F.interpolate(image_start, size=(224, 224))
        clip_img_feature = self.clip_img_model.encode_image(LDM_image_224)
        cond_clip = self.cond_stage_model.get_clip_feature(cond)
        clip_loss = torch.pow(clip_img_feature - cond_clip, 2).mean()
        loss_dict.update({f'loss_clip': clip_loss.mean()})
